...
const Navbar = () => {
  ...
  return (
    <nav className="z-50 fixed top-0 left-0 right-0 h-16 backdrop-blur-md flex items-center justify-between px-5 md:px-10">
      ...
      <input 
        type="text"
        className="hidden md:block w-64 md:w-80 h-8 border-2 border-gray-600 rounded-md px-2"
        placeholder="Search..."
      />
  ...
  <div className="absolute top-16 left-0 w-full bg-white shadow-md flex flex-col items-center py-5 space-y-4">
    ...
    <input 
      type="text"
      className="w-4/5 h-8 border-2 border-gray-600 rounded-md px-2"
      placeholder="Search..."
    />
    <Link className="text-lg font-semibold text-gray-800" to="/login">Log in</Link>
    <Link className="text-lg font-semibold text-gray-800" to="/register">Sign up</Link>
  </div>
)}
